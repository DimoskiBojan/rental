package mk.ukim.finki.emt.rental.sharedkernel.port.client;
...
public class RemoteEventLogServiceClient implements RemoteEventLogService {
    ...
    public RemoteEventLog currentLog(long lastProcessedId) {
        URI currentLogUri = UriComponentsBuilder.fromUriString(serverUrl)
                .path(String.format("/api/event-log/%d", lastProcessedId))
                .build()
                .toUri();
    ...
    private RemoteEventLog retrieveLog(@NonNull URI uri) {
        ResponseEntity<List<StoredDomainEvent>> response = restTemplate
                .exchange(uri, HttpMethod.GET, null, new ParameterizedTypeReference<List<StoredDomainEvent>>() {
                });
...
class RemoteEventLogImpl implements RemoteEventLog {
    private ResponseEntity<List<StoredDomainEvent>> response;
